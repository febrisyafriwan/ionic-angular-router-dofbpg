...
/**
 * Generated class for the FinishedContestMentoringPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-finished-contest-mentoring',
  templateUrl: 'finished-contest-mentoring.html',
})
export class FinishedContestMentoringPage {
  ...
  loadDatatable() {
            ...
            isActionDynamic: [
              {
                'label': 'Mentoring',
                'icon': 'book',
                'onClick': (row:any) => { this.openMentoringPage(row) }
              },
            ],
        ...
        lazyLoad : {
          url : this.urlServices.admin + 'contests/contests/mentoring/end/table',
          isGetUrl : true,
          keyLimit: 'limit',
          keyOffset : 'offset',
          keyData : 'content',
  ...
  openMentoringPage(row:any){
    let param = row;
    this.navCtrl.setRoot('ContestMentoringPage', { param:param } );
  }
